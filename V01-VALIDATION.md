...
### Post-module-split build

On 2026-08-10 the monolithic `main.rs` was split into a `freefm` library
(`error`, `cli`, `storage`, `auth`, `protocol`, `domain`, `plan`, `sync`,
`render`, `runner`) plus a thin binary that only parses arguments and runs the
library. All 35 tests (31 library + 4 TUI, 1 ignored child-process helper)
...
because the protocol and sync logic were moved verbatim.
...
was added. The local `codex-cli 0.141.0` was inspected directly. `codex exec`
and desktop recurring automations start an Agent turn (LLM tokens), so the
documented zero-token path stays with the OS scheduler or the deterministic
`codex sandbox -- freefm sync --quiet` runner. The sandbox runner could not be
executed inside this restricted environment (`sandbox_apply: Operation not
permitted`); separately, the user's `~/.codex/cc-switch-model-catalog.json`
currently fails Codex's parser (`unknown variant audio`) and should be
regenerated by cc-switch or removed before sandbox/exec commands can run.
Codex runtime verification therefore remains a user-local step.
...
As of 2026-08-10 13:33 Asia/Shanghai the hourly observation has produced 17
batches (first 2026-08-09 14:28 UTC, last 2026-08-10 05:22 UTC) with 51 salted
track hashes and 17 salted batch hashes, all unique, and no failure records.
Session checks are 15/15 successful, 15/15 authenticated, 15/15 explicit
`vipType=0`, each with `client_calls=1` and `http_requests=1`. The +24 hour
gate is 2026-08-10 23:21:34 Asia/Shanghai and the +7 day gate is 2026-08-16
23:21:34 Asia/Shanghai; the LaunchAgent keeps running until then.
